...
//写寄存器函数
//data:寄存器值
//通过 LCD_RS来标记是写入命令（LCD_RS=0）还是数据（LCD_RS=1）
//该函数是通过 8080 并口向 LCD 模块写入寄存器命令   使用频率不高
...
//写数据函数
//可以替代LCD_WR_DATAX宏,拿时间换空间.//功能和 LCD_WR_DATA一模一样
//data:寄存器值
...
//LCD_WriteReg 用于向 LCD 指定寄存器写入指定数据，而LCD_ReadReg 则用于读取指定寄存器的数据，这两个函数，都只带一个参数/返回值，
//所以，在有多个参数操作（读取/写入）的时候，就不适合用这两个函数了，
//写寄存器
//LCD_Reg:寄存器编号
//LCD_RegValue:要写入的值
//
